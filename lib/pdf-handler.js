    ...
    reader.onload = async (event) => {
      try {
        ...
        for (let i = 1; i <= pageCount; i++) {
          ...
          // Reconstruct text with column layout support (X-coordinate clustering)
          ...
          // Split items into columns if page width suggests two columns
          // Using midX as a rough divider. We add a small buffer (e.g., +20).
          const midX = (minX + maxX) / 2;
          let leftColumn = [];
          let rightColumn = [];
          
          items.forEach(item => {
             // For standard IEEE/Springer, left column is clearly left of midX
             if (item.transform[4] < midX + 15) {
          ...
          // Sort each column top-to-bottom (descending Y)
          // pdfjs transform: [scaleX, skewY, skewX, scaleY, translateX(x), translateY(y)]
          const sortItems = (a, b) => {
             const yDiff = b.transform[5] - a.transform[5];
             // If Y is roughly the same (same line), sort by X
             if (Math.abs(yDiff) < 5) {
          ...
          leftColumn.sort(sortItems);
          rightColumn.sort(sortItems);

          const reconstructText = (colItems) => {
            let colText = "";
            let lastY = null;
            for (let idx = 0; idx < colItems.length; idx++) {
              const item = colItems[idx];
              const currentY = item.transform[5];
              if (lastY !== null && Math.abs(currentY - lastY) > 5) {
                colText += "\n" + item.str;
              } else {
                colText += (colText === "" || colText.endsWith("\n") ? "" : " ") + item.str;
              }
              lastY = currentY;
            }
            return colText;
          ...
          // If there's barely any text in right column, it might be a single column page
          let text = "";
          if (rightColumn.length < 5) {
             items.sort(sortItems);
             text = reconstructText(items);
          } else {
             text = reconstructText(leftColumn) + "\n\n" + reconstructText(rightColumn);
          }
