...
const STOP_WORDS = new Set([
  ...
  'will', 'with', 'what', 'who', 'how', 'when', 'where', 'why', 'do', 'does',
  'did', 'can', 'could', 'would', 'should', 'this', 'there', 'their'
]);

/**
 * Clean and tokenize text (Stop words removed)
 * @param {string} text 
 * @returns {string[]}
 */
...
/**
 * Scores and retrieves top-K relevant chunks using Hybrid Search (TF-IDF + Embeddings)
 * @param {string} query The user's question
 * @param {Array<{id: string, content: string, page: number, section: string, embedding?: number[]}>} chunks All chunks for the paper
 * @param {number} topK Number of chunks to return
 * @returns {Promise<Array<{chunk: Object, score: number}>>}
 */
export async function retrieveChunks(query, chunks, topK = 4) {
  ...
  // Calculate Document Frequency (DF) for IDF calculation and build vocabulary
  ...
  // Spelling correction via Levenshtein Distance
  ...
  const correctedQueryTokens = queryTokens.map(token => {
    if (vocab.has(token)) return token; // Word exists in text
    let bestWord = token;
    let minDist = 3; // Max edit distance to consider
    for (const v of vocab) {
      if (Math.abs(v.length - token.length) > 2) continue; // Heuristic speedup
      const dist = levenshtein(token, v);
      if (dist < minDist) {
        minDist = dist;
        bestWord = v;
      }
  ...
  // 1. Generate Query Embedding for Semantic Search using corrected query
  let queryEmbedding = [];
  try {
     queryEmbedding = await generateEmbedding(correctedQueryTokens.join(' '));
  } catch (e) {
     console.warn("Embedding generation failed, falling back to TF-IDF only.", e);
  }
  ...
  // Calculate TF-IDF vector for Query (using corrected tokens)
  const queryTf = {};
  correctedQueryTokens.forEach(token => {
  ...
  // Calculate Hybrid Score (TF-IDF + Dense Embeddings)
  const ALPHA = 0.7; // Weight for Semantic Score (70% Semantic, 30% Keyword)
  ...
  const scored = docVectors.map(docVec => {
    ...
    const keywordScore = (queryLength > 0 && docVec.length > 0) 
      ? dotProduct / (queryLength * docVec.length) 
      : 0;

    // Semantic Embedding Score
    let semanticScore = 0;
    if (queryEmbedding.length > 0 && docVec.doc.embedding && docVec.doc.embedding.length > 0) {
       semanticScore = cosineSimilarity(queryEmbedding, docVec.doc.embedding);
    }
