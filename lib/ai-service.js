...
const DEFAULT_MODELS = {
  gemini: "gemini-2.5-flash",       // better free model, 1M context
  openai: "gpt-4o-mini",
...
/**
 * Retry an async function up to maxRetries times on rate-limit (429) errors.
 * Waits 2^attempt seconds between retries (exponential backoff).
 */
 ...
 */
async function buildChatModel(apiKey, provider = "gemini", modelOverride = null, temperature = 0.2, geminiJsonMode = false) {
...
${context}`;
  ...
  const chatModel = await buildChatModel(apiKey, provider, model, 0.2, provider === "gemini");
  const { HumanMessage } = await import("@langchain/core/messages");
  const response  = await withRetry(() => chatModel.invoke([new HumanMessage(prompt)]));
...
${context}`;
  ...
  const chatModel = await buildChatModel(apiKey, provider, model, 0.2);
  const { HumanMessage } = await import("@langchain/core/messages");
...
${ctx}`;
  ...
  const chatModel = await buildChatModel(apiKey, provider, model, 0.1, provider === "gemini");
  const { HumanMessage } = await import("@langchain/core/messages");
  const response  = await chatModel.invoke([new HumanMessage(prompt)]);
  const raw       = extractText(response);
...
export async function performRagChat(apiKey, model, query, contextChunks, history = [], provider = "gemini") {
  ...
  // REVERSE REPACKING (Phase 4): Reverse the chunks so the highest scored chunk is at the bottom, closest to the prompt
  cappedChunks.reverse();
...
${contextText}`;
  ...
  const chatModel = await buildChatModel(apiKey, provider, model, 0.1);
  const response  = await withRetry(() => chatModel.invoke(messages));
  const text      = extractText(response);
  if (!text) throw new Error("Empty response from model");
  return text;
}
