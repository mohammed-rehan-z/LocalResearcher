...
import { generateEmbedding, answerQuestion } from "../../lib/embedding-service";
...
function formatCitation(paper, format) {
  ...
  switch (format) {
    ...
    case "mdpi": {
      ...
      return `${mdpiAuthors}. ${title}. LocalResearcher ${year}.`;
    }
    case "researchgate":
...
export default function DashboardPage() {
  ...
  // Scraper States
  const [scraperTitle, setScraperTitle] = useState("");
  const [scraperTags, setScraperTags] = useState("");
  const [scraperTimeline, setScraperTimeline] = useState("");
  ...
  useEffect(() => {
    ...
    let savedUploaded = [];
    try {
      const raw = localStorage.getItem("LocalResearcher_uploaded_papers");
    ...
    let savedChats = {};
    try {
      const rawChats = localStorage.getItem("LocalResearcher_chat_histories");
  ...
  useEffect(() => {
    ...
    try {
      ...
      localStorage.setItem("LocalResearcher_uploaded_papers", payload);
    } catch (_) {}
  }, [papers]);
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    try {
      localStorage.setItem("LocalResearcher_chat_histories", JSON.stringify(chatHistories));
    } catch (_) {}
  }, [chatHistories]);
  ...
  const handleScraperSearch = async (e) => {
    e.preventDefault();
    if ((!scraperTitle.trim() && !scraperTags.trim()) || isScraping) return;
    ...
    try {
      const response = await fetch(`/api/search/stream?title=${encodeURIComponent(scraperTitle)}&tags=${encodeURIComponent(scraperTags)}&source=${scraperSource}&limit=${scraperLimit}`);
  ...
  const handleFileUpload = async (e, droppedFiles = null) => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        // Generate embeddings for each chunk locally
        ...
        if (apiKey) {
          try {
            ...
            summaryContent = await generateLocalSummary(allChunks, resolvedTitle, resolvedAuthors);
          }
        } else {
          metadata = extractLocalMetadata(allChunks, resolvedTitle);
          summaryContent = await generateLocalSummary(allChunks, resolvedTitle, resolvedAuthors);
  ...
  const handleGenerateSummary = async (paper) => {
    if (!paper) return;
    setSummarizingPaperId(paper.id);
    try {
      const meta = extractLocalMetadata(paper.chunks, paper.title);
      const summary = await generateLocalSummary(paper.chunks, paper.title, paper.authors);
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const retrieved = await retrieveChunks(cleanQuery, targetChunks, topK);
      ...
      let replyContent;
      const performLocalQA = async (query, chunks, targetPaper) => {
        let bestAnswer = null;
        let highestScore = -1;
        
        // Prepend a synthetic metadata chunk so QA can easily answer "who is the author", "what is the title", etc.
        const metaChunk = {
          content: `Paper Metadata - Title: ${targetPaper?.title || "Unknown"}. Authors: ${targetPaper?.authors || "Unknown"}. Year: ${targetPaper?.year || "Unknown"}.`
        };
        const searchChunks = [metaChunk, ...chunks.slice(0, 4)];

        for (const chunk of searchChunks) {
          const qaResult = await answerQuestion(query, chunk.content);
          if (qaResult && qaResult.score > highestScore) {
            highestScore = qaResult.score;
            bestAnswer = qaResult.answer;
          }
        }
        return bestAnswer && highestScore > 0.05 
          ? `**[Local QA]** ${bestAnswer}` 
      ...
      if (apiKey) {
        try {
          ...
          console.warn("AI chat failed, falling back to local QA:", aiErr.message);
          replyContent = await performLocalQA(cleanQuery, retrievedChunks, papers.find(p => p.id === chatScopeId) || activePaper);
        }
      } else {
        replyContent = await performLocalQA(cleanQuery, retrievedChunks, papers.find(p => p.id === chatScopeId) || activePaper);
      }
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute("download", "LocalResearcher_catalog.csv");
  ...
  const handleExportMarkdown = () => {
    let md = `# LocalResearcher Formulation Ledger\n\n`;
    ...
    link.setAttribute("download", "LocalResearcher_ledger.md");
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };
  ...
  const handleExportPDF = () => {
    ...
    let html = `
      <html>
        <head>
          <title>LocalResearcher Research Ledger</title>
          <style>
        ...
        <body>
          <h1>LocalResearcher Formulation Ledger</h1>
          <table>
            <thead>
              <tr>
                <th>Title</th>
  ...
  return (
    <div className="min-h-[100dvh] bg-background text-on-background flex flex-col flex-1 relative w-full">
      {/* ETHEREAL GLASS BACKGROUND MESH (GLOBAL) */}
      <div className="fixed inset-0 z-0 pointer-events-none opacity-20 opacity-40">
        <div className="absolute top-[10%] left-[-10%] w-[50%] h-[50%] rounded-full bg-[#38bdf8]/10 blur-[120px]" />
      ...
      {/* --- MAIN PAGE CONTENT --- */}
      <main className="w-full max-w-[1400px] mx-auto px-margin-mobile md:px-margin-desktop pt-16 md:pt-24 pb-16 flex-1">
            ...
            {activeView === "scraper" && (
              ...
              <div className="flex flex-col gap-8 pb-24 pt-8 max-w-6xl mx-auto">
                <div className="border-b border-outline pb-6">
                  <h1 className="font-serif text-4xl  text-primary tracking-tight mb-2">Literature Search</h1>
                  <p className="font-sans text-[17px] text-[#737373] text-on-surface-variant font-light">Search multiple academic databases simultaneously and ingest metadata directly into your library.</p>
                </div>

                <div className=" bg-surface-container-low border border-outline rounded-[1.5rem] shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] relative overflow-hidden backdrop-blur-xl p-6 md:p-8">
                  <form onSubmit={handleScraperSearch} className="flex flex-col md:flex-row gap-4 items-end">
                    <div className="flex-grow w-full grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        <label className="block font-label-sm text-label-sm text-on-surface-variant mb-2 uppercase tracking-widest">Title</label>
                        <input 
                          type="text" 
                          value={scraperTitle}
                          onChange={(e) => setScraperTitle(e.target.value)}
                      ...
                      <div>
                        <label className="block font-label-sm text-label-sm text-on-surface-variant mb-2 uppercase tracking-widest">Tags (Max 3, CSV)</label>
                        <input 
                          type="text" 
                          value={scraperTags}
                          onChange={(e) => setScraperTags(e.target.value)}
                          placeholder="e.g. LLM, RAG, AI"
                          className="w-full bg-surface-container border border-outline rounded-lg px-4 py-3 font-body-md text-body-md text-on-surface focus:border-on-surface-variant outline-none btn-press"
                        />
                      </div>
                      {/* Timeline filter moved to post-fetch results area */}
                    </div>
                ...
                {scraperResults.length > 0 && (
                  <div className="flex justify-end mb-4">
                    <div className="flex items-center gap-3">
                      <label className="font-label-sm text-label-sm text-on-surface-variant uppercase tracking-widest">Filter by Year:</label>
                      <select 
                        value={scraperTimeline}
                        onChange={(e) => setScraperTimeline(e.target.value)}
                        className=" bg-surface-container-low border border-outline rounded-[1.5rem] shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] relative overflow-hidden backdrop-blur-xl px-3 py-2 font-body-md text-body-md text-on-surface outline-none cursor-pointer"
                      >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {scraperResults
                    .filter(result => !scraperTimeline || (result.date && result.date.startsWith(scraperTimeline)))
                    .map((result, idx) => (
                    <div key={idx} className=" bg-surface-container-low border border-outline rounded-[1.5rem] shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] relative overflow-hidden backdrop-blur-xl flex flex-col card-hover overflow-hidden group">
            ...
            {activeView === "dashboard" && (
              ...
              <div className="flex flex-col gap-8 pb-24 pt-8 max-w-7xl mx-auto">
                ...
                <div className="border-b border-outline pb-6">
                  <h1 className="font-serif text-4xl  text-primary tracking-tight mb-2">Research Library</h1>
                  <p className="font-sans text-[17px] text-[#737373] text-on-surface-variant font-light">Upload and catalog academic research papers, index text chunks, and perform RAG operations.</p>
                </div>

                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  {/* Left Column: Upload Block */}
                  <div className="lg:col-span-4 flex flex-col gap-6">
                    <div className=" bg-surface-container-low border border-outline rounded-[1.5rem] shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] relative overflow-hidden backdrop-blur-xl p-6">
                    ...
                    <div className="flex flex-col md:flex-row gap-4">
                      <div className="relative flex-grow">
                        ...
                        <input 
                          ...
                          className="w-full  bg-surface-container-low border border-outline rounded-[1.5rem] shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] relative overflow-hidden backdrop-blur-xl focus:border-outline-variant p-3 pl-10 outline-none font-body-md text-body-md text-on-surface btn-press"
                          value={searchQuery}
                      ...
                      {allTags.length > 0 && (
                        <select 
                          className=" bg-surface-container-low border border-outline rounded-[1.5rem] shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] relative overflow-hidden backdrop-blur-xl focus:border-outline-variant p-3 outline-none font-body-md text-body-md text-on-surface cursor-pointer btn-press appearance-none md:min-w-[200px]"
                          value={selectedTag}
              ...
              <div className="">
                <div className="mb-8 border-b border-outline pb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
                  <div className="max-w-4xl">
                    <h1 className="font-serif text-4xl  text-primary tracking-tight mb-1">
                      {activePaper.title}
                    </h1>
                ...
                <div className="swiss-grid">
                  ...
                  <div className="col-span-12 lg:col-span-4 flex flex-col gap-6">
                    <div className=" bg-surface-container-low border border-outline rounded-[1.5rem] shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] relative overflow-hidden backdrop-blur-xl p-6 flex flex-col gap-6">
                    ...
                    <div className=" bg-surface-container-low border border-outline rounded-[1.5rem] shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] relative overflow-hidden backdrop-blur-xl p-6 flex flex-col gap-4">
                  ...
                  {/* Right Panel: Summary */}
                  <div className="col-span-12 lg:col-span-8  bg-surface-container-low border border-outline rounded-[1.5rem] shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] relative overflow-hidden backdrop-blur-xl p-6 md:p-8 flex flex-col gap-6">
              ...
              <div className="">
                <div className="mb-8 border-b border-outline pb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
                  <div className="max-w-4xl">
                    ...
                    <h1 className="font-serif text-4xl  text-primary tracking-tight mb-2">Q&A Chat</h1>
                ...
                <div className="swiss-grid h-auto lg:h-[680px]">
                  ...
                  <div className="col-span-12 lg:col-span-4  bg-surface-container-low border border-outline rounded-[1.5rem] shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] relative overflow-hidden backdrop-blur-xl p-6 flex flex-col gap-4 h-full overflow-hidden">
                    ...
                    <p className="font-body-md text-body-md text-on-surface-variant leading-relaxed mb-2">
                      LocalResearcher parses matching segments using client-side indexing. Below are the indexed text passages that support factual verification.
                    </p>
                  ...
                  <div className="col-span-12 lg:col-span-8  bg-surface-container-low border border-outline rounded-[1.5rem] shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] relative overflow-hidden backdrop-blur-xl p-6 flex flex-col justify-between h-[550px] lg:h-full overflow-hidden">
                    ...
                    <div className="flex-grow overflow-y-auto pr-2 flex flex-col gap-6 mb-6">
                      ...
                      <div className="border border-outline bg-surface-container p-5 animate-fade-in border-l-4 border-l-accent rounded">
                        ...
                        <div className="font-body-md text-body-md text-on-surface-variant leading-relaxed">
                          {chatScopeId === "all" ? (
                            <span>Welcome to the Global LocalResearcher Q&A. I will analyze questions across <strong className="text-primary">all {papers.length} papers</strong> indexed in your library.</span>
                          ) : (
                            <span>Welcome to LocalResearcher. Q&A workspace successfully created for <strong className="text-primary">&ldquo;{papers.find(p => p.id === chatScopeId)?.title || activePaper?.title}&rdquo;</strong>. Ask any target question.</span>
                          )}
                        </div>
                          ...
                          <span className="font-label-sm text-label-sm uppercase tracking-wider font-bold flex items-center gap-1.5 mb-2">
                            {msg.role === "user" ? (
                              ...
                              <span className="text-accent flex items-center gap-1.5">
                                <BookOpen size={12} />
                                <span>LocalResearcher Terminal</span>
                              </span>
                            )}
                          </span>
                      ...
                      {chatLoading && (
                        <div className="border border-outline bg-surface-container p-5 mr-12 animate-pulse border-l-4 border-l-accent rounded">
                          <span className="font-label-sm text-label-sm uppercase tracking-wider text-accent font-bold flex items-center gap-1.5 mb-2">
                            <BookOpen size={12} />
                            <span>LocalResearcher is searching local vectors...</span>
                          </span>
              ...
              <div>
                <div className="mb-8 border-b border-outline pb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
                  <div className="max-w-4xl">
                    ...
                    <h1 className="font-serif text-4xl  text-primary tracking-tight mb-2">Structured Catalog</h1>
                    <p className="font-sans text-[17px] text-[#737373] text-on-surface-variant font-light">A horizontal ledger tracking methodology, extracted results, contributions, and datasets for cross-paper evaluation.</p>
                  </div>
                ...
                <div className=" bg-surface-container-low border border-outline rounded-[1.5rem] shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] relative overflow-hidden backdrop-blur-xl p-5 mb-6 flex flex-col gap-3">
            ...
            {activeView === "settings" && (
              ...
              <div>
                <div className="mb-8 border-b border-outline pb-6">
                  <h1 className="font-serif text-4xl  text-primary tracking-tight mb-2">System Settings</h1>
                  <p className="font-sans text-[17px] text-[#737373] text-on-surface-variant font-light">Configure RAG window sizing and vector chunk properties. Fully local — no API keys required.</p>
                </div>

                <div className="max-w-[700px]  bg-surface-container-low border border-outline rounded-[1.5rem] shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] relative overflow-hidden backdrop-blur-xl p-6 md:p-8">
